 ...
 *  关于数据类成员的继承要点：
 *
 *  1. 如果数据类中显式定义了equals, hashCode或是toString方法，或是在数据类的父类中将这些方法声明为final，
 *     那么这些方法将不会再生成，转而使用已有的。
 ...
 *  解构声明
 *
 *  在主构造方法中有多少个参数，就会依次生成component1, component2, component3...
 *  这些方法返回的就是对应字段（声明顺序）的值，componentN方法是用来实现解构声明的
 */
data class Person(val name: String, var age: Int, var address: String)

/**
 *  在jvm上，如果生成的类要拥有无参的构造方法，那么就需要为所有属性指定默认值
 */
data class Person2(val name: String = "", var age: Int = 22, var address: String = "beijing")

fun main() {
    val person = Person("zhangsan", 22, "beijing")
    println(person)
