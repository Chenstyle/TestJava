 ...
 *  关于属性委托的要求
 *
 *  对于只读属性（val修饰的属性）来说，委托需要提供一个名为getValue的方法，该方法接收如下参数：
 *  - thisRef，需要是属性拥有者相同的类型或是其父类型（对于扩展属性来说，这个类型指的是被扩展的那个类型）
 *  - property，需要是KProperty<*>类型或是其父类型
 *
 *  对于可变属性（var修饰的属性）来说，委托需要再提供一个名为setValue的方法，该方法需要接收如下参数：
 ...
 *
 *  这两个方法都必须要标记位operator关键字。对于委托类来说，它可以实现ReadOnlyProperty或是ReadWriteProperty接口，
 *  这些接口包含了相应的getValue与setValue方法；同时，对于委托类来说，也可以不去实现这两个接口，而是自己单独实现相应的
 *  getValue与setValue方法
 *
 *  委托转换规则：
 *
 *  对于每个委托属性来说，Kotlin编译器在底层会生成一个辅助的属性，然后将对原有属性的访问委托给这个属性。
